...
class Classe_Dataset:
    ...
    def acquisisci(self, percorsohdf5):  # colselezione faccio selezione solo dei dati che servono
        ...
        for i in range(len(nomidata)):
            if self.allmetadata["trace_polarity"][i] != 'undecidable' \
                    and (self.allmetadata["station_channels"][i]=="HH" or self.allmetadata["station_channels"][i]=="EH"  ):        # TODO condizione da aggiornare
        ...
        sismogramma = np.array(self.sismogramma)
        print("shape", sismogramma.shape, len(self.metadata["trace_P_arrival_sample"]))
...
Dataset_1.acquisisci(hdf5)
Dataset_1.plotta(visualizza=1058)
