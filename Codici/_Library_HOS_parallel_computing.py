...
# UPGRADE: Now i can examine multiple traces all at once!
...
def sliding_window_view(arr, window_shape, steps):
    ...
    windowed = as_strided(arr, shape=outshape, strides=strides, writeable=False)
    if len(windowed.shape) == 3:
        reshape1 = windowed.reshape((windowed.shape[0]*windowed.shape[1],windowed.shape[2]),order="F")
        windowed = reshape1.reshape((windowed.shape[1],windowed.shape[0],windowed.shape[2]))
...
def S_6(data, axis=-1):
    # can be very slow
    return np.sum((data-np.expand_dims(np.mean(data,axis=axis),axis=-1))**6,axis=axis)/(data.shape[-1]-1)/np.std(data,ddof=1,axis=axis)**6-15
